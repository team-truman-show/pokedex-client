...
const BringUp = () => {
  ...
  const handleEvloutionClick = async () => {
    ...
    const evolve = await pokeEvolve(mypokeid, pokemonid);
    // console.log(evolve);
    if (!evolve.id || !evolve.pokeid);
    window.location.href = `/bringup/${evolve.id}/${evolve.pokeid}`;
    // setModalContent({
    //   title: '진화하기',
    //   message: '앗 포켓몬 상태가?',
    //   isError: false,
